...
#####Iterative#####  def MC_Iterative(self, iterations, alpha, epsilon=0, update_epsilon=True, gamma=gamma_cid)
###################
print('Begin MC ITERATIVE')
grid = GridWorld()
repetitions = 300
iterations = 1000
alphas = [0.1, 0.2]
for graph in range(len(alphas)):
    ...
    with Pool(cores) as pool:
        Values_noUpdate = np.array(pool.starmap(grid.MC_Iterative, [(iterations,alphas[graph],0.05,False) for rep in range(repetitions)] ))

    print(1)

    with Pool(cores) as pool:
        Values_Update1 = np.array(pool.starmap(grid.MC_Iterative, [(iterations,alphas[graph],0.05,True) for rep in range(repetitions)] ))

    print(2)

    with Pool(cores) as pool:
        Values_Update2 = np.array(pool.starmap(grid.MC_Iterative, [(iterations,alphas[graph],0.15,True) for rep in range(repetitions)] ))

    print(3)


    file = 'MC_iter_{}'.format(graph)
    with open(file, 'wb') as f:
        pickle.dump([Values_noUpdate,Values_Update1,Values_Update2], f)
        f.close()
        print('saved')

print('ALL DONE MC ITERATIVE')
